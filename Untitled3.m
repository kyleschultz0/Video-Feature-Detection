...
for i = 1:total_bolts
    ...
    eval(sprintf('Boltadjust(:, :, i) = rgb2gray(Boltcropped_%d(qx+rx+1:sz(1)-qx,qy+ry+1:sz(2)-qy , :));', i, i));   
end
...
bolts_reshaped = reshape(Boltadjust, [minx*miny, total_bolts]);
...
for j = 8
    ...
    im_rank1s = reshape(im_rank1, [minx, miny, total_bolts]);
    for i = 1:total_bolts
        subplot(8,8,i)
...
for i = 1:numframes
    ...
    for j = 0:numwindows_y-2
        for k = 0:numwindows_x-2
            window = rgb2gray(frame(k*shift_x+1:k*shift_x+window_x, j*shift_y+1:j*shift_y+window_y, :));
            window_reshape = double(reshape(window, [minx*miny, 1]));
